...
if __name__ == "__main__":
    ...
    total_it_power = np.sum(LNM) + np.sum(LMS_ori) + (175.39 * 12 * 8) + (213.2503125 * 80 * 4 + 213.2503125 * 48 * 4)
    ...
    for s in starts_lmc:
        for j in range(12): 
            LMC_base[(s + j) % T] += 175.39

    starts_lmi_80 = [56, 64, 72, 80] 
    for s in starts_lmi_80:
        for j in range(80):
            LMI_base[(s + j) % T] += 213.2503125
    ...
    plt.bar(x, LMS_ori, width=0.25, bottom=LNM, label='LMS', color=colors_12['LMS'], alpha=0.9, edgecolor='none')
    plt.bar(x, LMC_base, width=0.25, bottom=LNM+LMS_ori, label='LMC', color=colors_12['LMC'], alpha=0.9, edgecolor='none')
    plt.bar(x, LMI_base, width=0.25, bottom=LNM+LMS_ori+LMC_base, label='LMI', color=colors_12['LMI'], alpha=0.9, edgecolor='none')
    plt.xlabel('Time (h)', fontweight='bold'); plt.ylabel('IT Power (kW)', fontweight='bold')
    plt.xlim(0, 24)

    plt.legend(loc='upper center', ncol=4, frameon=True, fontsize=9, framealpha=0.95, edgecolor='gray')
    
    plt.grid(False); plt.savefig('Fig2_Baseline_Workload.png', dpi=300, bbox_inches='tight')
    ...
    plt.plot(x, PV+Wind, color=colors_12['RES'], linestyle='--', linewidth=2.5, label='Total RES Generation')
    plt.xlabel('Time (h)', fontweight='bold'); plt.ylabel('IT Power (kW)', fontweight='bold')
    plt.xlim(0, 24)
    plt.legend(loc='upper right', frameon=True, fontsize=9, framealpha=0.95, edgecolor='gray')
    plt.grid(False); plt.savefig('Fig3_Optimized_Workload.png', dpi=300, bbox_inches='tight')
    ...
    plt.grid(False); plt.savefig('FigA1_RES_Profile.png', dpi=300, bbox_inches='tight')
    ...
    plt.grid(False); plt.savefig('FigA2_Price_Curve.png', dpi=300, bbox_inches='tight')
    ...
    ax1.legend(handles1 + handles2, labels1 + labels2, 
               ...
               ncol=3, frameon=True, fontsize=10, framealpha=0.95, edgecolor='gray')
    plt.savefig('Fig4_VES_Effect.png', dpi=300, bbox_inches='tight')


    time_labels = [f"{int(t*0.25):02d}:{int((t*0.25%1)*60):02d}" for t in range(T)]
    
    df_baseline = pd.DataFrame({
        'Time_Step': range(1, T+1),
        'Time_Clock': time_labels,
        'LNM_kW (Rigid)': LNM,
        'LMS_kW (Short-term)': LMS_ori,
        'LMC_kW (Continuous)': LMC_base,
        'LMI_kW (Interruptible)': LMI_base,
        'Total_IT_Power_kW': P_IT_base
    })
    
    df_baseline = df_baseline.round(2)
    
    csv_file_name = 'Baseline_Schedule_96Steps.csv'
    df_baseline.to_csv(csv_file_name, index=False)
    
    print(f"\nBaseline schedule CSV generated: {csv_file_name}")
