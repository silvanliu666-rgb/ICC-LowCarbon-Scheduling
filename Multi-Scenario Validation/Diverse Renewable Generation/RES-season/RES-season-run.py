...
if __name__ == "__main__":
    ...
    df_res = pd.read_excel('RES-season.xlsx', sheet_name='季节')
    
    scenarios = {
        'Spring': {'PV': 'Spring_PV', 'Wind': 'Spring_Wind'},
        'Summer': {'PV': 'Summer_PV', 'Wind': 'Summer_Wind'},
        'Autumn': {'PV': 'Autumn_PV', 'Wind': 'Autumn_Wind'},
        'Winter': {'PV': 'Winter_PV', 'Wind': 'Winter_Wind'}
    }
    ...
    print("\nStarting evaluation of 4 seasonal scenarios...")
    ...
    for sc_name, cols in scenarios.items():
        ...
        plt.ylabel('IT Power (kW)', fontweight='bold')
        plt.xlim(0, 24)
        plt.legend(loc='upper right', frameon=True, fontsize=11, framealpha=0.95, edgecolor='gray')
        plt.grid(False)
    ...
    print("\n" + "="*85)
    print(" [Season Optimization Results Summary]")
    print("="*85)
    print(df_results.to_markdown(index=False))
    print("="*85 + "\n")
    
    df_results.to_csv('Optimization_Results_Season_Summary.csv', index=False)
    print("All seasonal scenarios evaluated! Data saved to 'Optimization_Results_Season_Summary.csv'.")